...
static GOptionEntry option_entries[] = {
    ...
    {"port", 'p', 0, G_OPTION_ARG_INT, &listen_port,
     ...
     "SO_REUSEADDR, so might increment)",
     NULL},
    {"uuid", 'u', 0, G_OPTION_ARG_STRING, &uuid, "UUID to advertise", NULL},
    {"friendly-name", 'f', 0, G_OPTION_ARG_STRING, &friendly_name,
     "Friendly name to advertise.", NULL},
    {"output", 'o', 0, G_OPTION_ARG_STRING, &output, "Output module to use.",
     NULL},
...
static const char *GetVersionInfo(char *buffer, size_t len) {
  ...
  snprintf(buffer, len,
           ...
           GM_COMPILE_VERSION, UPNP_VERSION_STRING, GLIB_MAJOR_VERSION,
           GLIB_MINOR_VERSION, GLIB_MICRO_VERSION, GST_VERSION_MAJOR,
...
int main(int argc, char **argv) {
  ...
  if (listen_port != 0 && (listen_port < 49152 || listen_port > 65535)) {
    ...
    Log_error("main",
              "Parameter error: --port needs to be in "
